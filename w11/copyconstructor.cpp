/*
CH-230-A
a10p7.cpp
Lars Schuster
lschuster@jacobs-university.de
*/

#include <iostream>
#include <cstring>  // standard C string.h header file

using namespace std;

/*
	Naive string: the goal is to illustrate how copy constructors 
	should be handled. See also charbuffer.cpp
*/

class NaiveString {
private:
	char *str;
public:
	NaiveString(const char*);
	NaiveString(const NaiveString&);
	~NaiveString();//prototype of Destructor
	void update(char, char);
	void print();
};

NaiveString::NaiveString(const char* t) {
	cout << "Calling normal constructor" << endl;
	str = new char[strlen(t) + 1];
	strcpy(str, t);
}

/* 
	Copy constructor: creates a new instance of 
	naive string starting from an existing one 
*/


NaiveString::NaiveString(const NaiveString& src) {
	cout << "Calling copy constructor" << endl;
	str = new char[strlen(src.str) + 1];
	strcpy(str, src.str);
	// please note that we can access src.str even if it is private
	// as we are inside the NaiveString class
}

/* replaces occurrences of oldchar by newchar */
void NaiveString::update(char oldchar, char newchar) {
	unsigned int i;
	for (i = 0; i < strlen(str); i++) 
		if (str[i] == oldchar)
			str[i] = newchar;
}
NaiveString::~NaiveString()
{//Destructor to free memory automatically
	delete [] str;
}

void NaiveString::print() {
	cout << str << endl;
}

//call by value does not chage the string
void funcByVal(NaiveString s) {
	cout << "funcbyval() being called" << endl;
	s.update('B', 'C');
	s.print();
}

//call by reference actually changes the initial string
void funcByRef(NaiveString &s) {
	cout << "funcbyref() being called" << endl;
	s.update('B', 'C');
	s.print();
}

int main(int argc, char** argv)
{
	NaiveString a("aBcBdB");//create a 
	NaiveString b (a);//create copy of a

	/*Call both functions on a*/
	a.print();
	cout << "\nAbout to call funcbyval()" << endl;
	funcByVal(a);
	a.print();
	cout << "\nAbout to call funcbyref()" << endl;
	funcByRef(a);
	a.print();
	cout << endl;
	/*Call both functions on b*/
	b.print();
	cout << "\nAbout to call funcbyval()" << endl;
	funcByVal(b);
	b.print();
	cout << "\nAbout to call funcbyref()" << endl;
	funcByRef(b);
	b.print();
}
